// Fetch-intercepted unit tests for EvriConnector -- no real Sapient/Evri
// credentials exist anywhere in this codebase yet (see evri-connector.ts's
// own class doc comment for the full CONFIRMED/INFERRED research trail:
// this connector integrates via the Sapient/Intersoft CORE API gateway,
// not a direct Evri API, since Evri publishes none). Same
// "stub globalThis.fetch, restore in finally" discipline every other
// connector test file in this codebase already uses (e.g.
// royal-mail-connector.test.ts).
//
// Run with: npm run test --workspace=@alltix/carrier-connectors

import { test } from "node:test";
import assert from "node:assert/strict";
import { EvriConnector, type EvriCredentials } from "../src/evri-connector.js";
import type { CreateShipmentRequest } from "../src/connector.js";

const CREDENTIALS: EvriCredentials = {
...
test("authenticate exchanges client_id/client_secret via HTTP Basic auth for a bearer token", async () => {
  ...
  try {
    const connector = new EvriConnector(CREDENTIALS);
...
test("createShipment sends a Bearer-authorized POST /v4/shipments/evri with the confirmed top-level shape", async () => {
  const originalFetch = globalThis.fetch;
  let capturedUrl = "";
  let capturedAuth = "";
  ...
  globalThis.fetch = stubTokenThenOnce(async (input, init) => {
    ...
    return jsonResponse(200, {
      ShipmentId: "SAP-12345",
      TrackingNumber: "EVR123456789GB",
  ...
  try {
    ...
    assert.equal(capturedUrl, "https://api.intersoftsapient.net/v4/shipments/evri");
    ...
    assert.equal(result.carrierOrderId, "SAP-12345");
    assert.equal(result.trackingNumber, "EVR123456789GB");
...
test("EvriConnector has no voidShipment -- not every carrier's API supports cancellation the same way", () => {
  const connector = new EvriConnector(CREDENTIALS);
...
test("trackShipment posts to /v4/trackings and maps the matching entry", async () => {
  ...
  globalThis.fetch = stubTokenThenOnce(async (input, init) => {
    ...
    capturedBody = JSON.parse(String(init?.body));
    return jsonResponse(200, {
      TrackingNumbers: [{ TrackingNumber: "EVR123456789GB", ShipmentId: "SAP-12345", Status: "Delivered" }],
    });
  });

  try {
    const connector = new EvriConnector(CREDENTIALS);
    const result = await connector.trackShipment("EVR123456789GB");
    assert.equal(capturedUrl, "https://api.intersoftsapient.net/v4/trackings");
    assert.deepEqual(capturedBody, { TrackingNumbers: ["EVR123456789GB"] });
...
test("getRateEstimate returns an empty list -- no confirmed Evri/Sapient surcharge or base-price data exists", async () => {
  const originalFetch = globalThis.fetch;
  globalThis.fetch = (async () => {
